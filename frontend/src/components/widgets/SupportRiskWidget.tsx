...
import { Headphones, Clock, TrendingUp, TrendingDown, Minus, MessageSquare, ThumbsUp, ThumbsDown, ChevronDown, ChevronRight, AlertCircle, CheckCircle, Loader, Filter, Info, BarChart3, ChevronLeft } from 'lucide-react'
import { BaseWidget, Badge } from './BaseWidget'
import { clsx } from 'clsx'
import type { SupportAnalysis, SupportTicket, ResolutionStats } from '../../types'
...
function TicketCard({ ticket }: { ticket: SupportTicket }) {
  ...
  const getStatusIcon = (status: string) => {
    switch (status) {
      ...
      case 'in_progress': return <Loader className="w-3 h-3 text-blue-500" />
      case 'resolved': return <CheckCircle className="w-3 h-3 text-emerald-500" />
      default: return null
    }
  }
  ...
  const getStatusTooltip = (status: string) => {
    switch (status) {
      ...
      case 'in_progress': return 'In Progress - being worked on'
      case 'resolved': return 'Resolved - ticket closed'
  ...
  const getSeverityColor = (severity: string) => {
    switch (severity) {
      case 'critical': return 'border-l-rose-500 bg-rose-50/50'
  ...
  return (
    <div className={clsx(
      'border-l-2 rounded-r-lg p-2 transition-all',
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-1.5 mb-1 flex-wrap">
            <span 
              ...
              title={getStatusTooltip(ticket.status)}
            >
      ...
      {ticket.total_messages !== undefined && ticket.total_messages > 0 && (
        <div className="flex items-center gap-3 mt-2 text-[10px] text-slate-500">
          <div 
            className="flex items-center gap-1"
            title="Total messages in conversation"
          >
      ...
      {expanded && ticket.summary && (
        <div className="mt-2 p-2 bg-white/80 rounded border border-slate-200">
          ...
          {/* Sentiment breakdown */}
          {(ticket.positive_messages || ticket.negative_messages || ticket.neutral_messages) && (
            <div className="flex items-center gap-3 mt-2 pt-2 border-t border-slate-100 text-[10px]">
              <span className="text-emerald-600" title="Positive sentiment messages">👍 {ticket.positive_messages || 0}</span>
              <span className="text-rose-600" title="Negative sentiment messages">👎 {ticket.negative_messages || 0}</span>
              <span className="text-slate-500" title="Neutral messages">😐 {ticket.neutral_messages || 0}</span>
            </div>
          )}
        </div>
      )}
